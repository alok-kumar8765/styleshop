...
def add_to_cart(request):
    ...
    product = Product.objects.get(id=product_id) #to match foregn key for instance
...
def mobile(request,data=None):
    if data == None:
        mobiles = Product.objects.filter(category='M')
    elif data == 'Redmi' or data == 'Xiaomi':
...
#def customerregistration(request): #function based but used to display static form
...
def payment_done(request):
    ...
    for c in cart:
        orderPlaced=Customer(user=user,product=Cart.product, quantity=c.quantity)
        orderPlaced.save()
        c.delete()
...
class ProfileView(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            reg.save()
            messages.success(request,'Conratulation data saved successfully')
        return render(request,'app/profile.html',{'form':form,'active':'btn-primary'})